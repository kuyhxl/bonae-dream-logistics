...
class HubRoutePathCacheFailureIntegrationTest {

    @DynamicPropertySource
    static void useUnavailableRedis(DynamicPropertyRegistry registry) {
        // 사용하지 않는 포트로 연결해 경로 결과/활성 그래프 캐시 장애를 함께 재현한다.
    ...
    void setUp() {
        ...
        departureHub = Hub.create("경로장애출발허브-" + suffix, "경로장애출발주소-" + suffix, 37.4742027808565, 127.123621185562);
        arrivalHub = Hub.create("경로장애도착허브-" + suffix, "경로장애도착주소-" + suffix, 37.1896213142136, 127.375050006958);
    ...
    @Test
    @DisplayName("Redis 조회와 저장이 실패해도 다익스트라 경로 결과를 반환한다")
    void returnsPathWhenRedisIsUnavailable() {
        ...
        assertThat(result.getTotalDistanceMeters()).isEqualTo(hubRoute.getDistanceMeters().longValue());
        assertThat(result.getTotalDurationSeconds()).isEqualTo(hubRoute.getDurationSeconds().longValue());
    }

    @Test
    @DisplayName("Redis 무효화가 실패해도 이동정보 수정은 정상 완료한다")
    void completesUpdateWhenPathCacheEvictionFails() {
