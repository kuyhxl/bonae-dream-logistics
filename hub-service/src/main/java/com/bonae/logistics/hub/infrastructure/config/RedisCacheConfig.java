...
public class RedisCacheConfig implements CachingConfigurer {
    ...
    private static final String HUB_PATH_CACHE_NAME = "hubPath";
    ...
    public RedisCacheManager redisCacheManager(RedisConnectionFactory connectionFactory, ObjectMapper objectMapper) {
        ...
        Jackson2JsonRedisSerializer<HubDetailResponse> hubDetailSerializer = new Jackson2JsonRedisSerializer<>(objectMapper, HubDetailResponse.class);

        RedisCacheConfiguration hubDetailConfig = defaultConfig
                .serializeValuesWith(RedisSerializationContext.SerializationPair.fromSerializer(hubDetailSerializer))
        ...
        Jackson2JsonRedisSerializer<List<HubRouteEdge>> hubRouteGraphSerializer = new Jackson2JsonRedisSerializer<>(objectMapper, hubRouteGraphType);
        ...
        Jackson2JsonRedisSerializer<HubRoutePathResponse> hubPathSerializer = new Jackson2JsonRedisSerializer<>(objectMapper, HubRoutePathResponse.class);

        RedisCacheConfiguration hubPathConfig = defaultConfig
                .serializeValuesWith(RedisSerializationContext.SerializationPair.fromSerializer(hubPathSerializer))
                .entryTtl(Duration.ofMinutes(5))
                // 출발/도착 허브 조합별 최적 경로 결과를 별도 키로 관리한다.
                .computePrefixWith(cacheName -> "hub:path:");
        ...
        return RedisCacheManager.builder(connectionFactory)
                ...
                .withCacheConfiguration(HUB_PATH_CACHE_NAME, hubPathConfig)
                .transactionAware() // 조회는 즉시 수행하고, 저장/삭제는 DB 트랜잭션 커밋 이후 반영한다.
                .build();
    }
