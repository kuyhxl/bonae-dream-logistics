...
public class DeliveryService {
    ...
    public ResGetDeliveryDto getDelivery(UUID deliveryId) {
        Delivery delivery = deliveryRepository.findByIdAndDeletedAtIsNull(deliveryId)
                .orElseThrow(() -> new BusinessException(ErrorCode.DELIVERY_NOT_FOUND));
        return ResGetDeliveryDto.from(delivery);
    }

    @Transactional(readOnly = true)
    public PageResponseDto<ResGetDeliveryListDto> getDeliveries(PageRequestDto pageRequestDto) {
        Page<Delivery> deliveries = deliveryRepository.findAllByDeletedAtIsNull(pageRequestDto.toPageable());
        return PageResponseDto.from(deliveries, ResGetDeliveryListDto::from);
    }
}
